...
globs:
---
# prepf - Technical Interview Coach CLI
...
- **Styling (`lipgloss`):**
  ...
  - **Never** use `for` loops with string concatenation for padding/borders.
  - For viewports: Apply padding to content in `SetContent()` using `lipgloss.Style.Padding().Render()`, not in `View()` (padding View() output breaks scrolling).
  - For other components: Apply padding declaratively in `View()` using `lipgloss.Style.Padding()`.
...
- **Slice/Map Mutation:** When passing mutable data structures (slices, maps) to functions that mutate them (e.g., form libraries), pass pointers `*[]T`, `*map[K]V`. Passing by value creates local copies; mutations to the copy don't affect the original.
