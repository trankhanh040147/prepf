...
import (
	...
	// Event package removed - prepf doesn't use event tracking
...
func RecoverPanic(name string, cleanup func()) {
	if r := recover(); r != nil {
		// Log panic (event tracking removed for prepf)
		slog.Error("Panic recovered", "name", name, "panic", r)

		// Create a timestamped panic log file
		timestamp := time.Now().Format("20060102-150405")
		filename := fmt.Sprintf("prepf-panic-%s-%s.log", name, timestamp)
